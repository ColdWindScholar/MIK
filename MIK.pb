...
Procedure FirmwareUnpack(Parameter$)
  ...
  If FileSize(WorkingDirectory$ + FileName$ + ".md5") > 0
    ...
    Err = RunConsole(bin$ + "md5sum.exe", "-c " + Chr(34) + FileName$ + ".md5" + Chr(34), WorkingDirectory$, #PB_Program_Wait)
...
Procedure FirmwarePack(Parameter$)
  ...
  If Err = 0
    ...
    If st\Get_Hash = #PB_Checkbox_Checked
      ...
      Err = RunConsole(bin$ + "md5sum.exe", "-t " + Chr(34) + NewFileName$ + Chr(34) + " > " + Chr(34) + GetFilePart(NewFileName$, #PB_FileSystem_NoExtension) + ".md5" + Chr(34), WorkingDirectory$, #PB_Program_Wait)
  ...
  Procedure DtbMultiUnpack(Parameter$, Sign$)
    ...
    Procedure DtbPack(Parameter$)
      ...
      Procedure tarUnpack(Parameter$)
        ...
        Message(Lng(32) + " " + FileNameExpansion$ + " " + Lng(30)) ;Выполняется распаковка образа прошивки: Подождите...
        Err = RunConsole(bin$ + "tar.exe", "-xvf " + Chr(34) + FileNameExpansion$ + Chr(34) + " -C " + Chr(34) + DirectoryName$ + Chr(34), WorkingDirectory$, #PB_Program_Wait)
        Message(Lng(42) + " tar: " + Str(Err) + ExitStr(Err)) ;Exitcode
        Err = RunConsole(bin$ + "ls.exe", "-l", ExtractionPath$, #PB_Program_Wait)
        ...
        Procedure tarPack(Parameter$)
          ...
          Err = RunConsole(bin$ + "tar.exe", "-cvf " + Chr(34) + "..\"+DirectoryName$+Exp$+Chr(34)+" -T "+Chr(34)+"tarFileList.cfg"+Chr(34), Parameter$, #PB_Program_Wait)
          Message(Lng(42) + " tar: " + Str(Err) + ExitStr(Err)) ;Exitcode
          If Err = 0
            Recent(Parameter$)
            If FileSize(WorkingDirectory$ + DirectoryName$ + Exp$ + ".md5") >= 0
              ;..\bin\md5sum.exe -t %file% >> %file%
              Err = RunConsole(bin$ + "md5sum.exe", "-t " + Chr(34) + DirectoryName$ + Exp$ + Chr(34) + " >> " + Chr(34) + DirectoryName$ + Exp$ + Chr(34), WorkingDirectory$, #PB_Program_Wait)
              Message(Lng(42) + " md5sum: " + Str(Err) + ExitStr(Err)) ;Exitcode
              If Err = 0
        ...
        Procedure MainUnpack(Parameter$)
          ...
          If FileSize(Parameter$) > 0
            ...
            If CheckSignature(ExtractionPath$ + "." + Expansion$) = "Erofs"
              ...
              Err = RunConsole(bin$ + "erofs\extract.erofs.exe", "-i " + Chr(34) + DirectoryName$ + "." + Expansion$ + Chr(34) + " -x -f -o " + Chr(34) + DirectoryName$ + Chr(34), WorkingDirectory$, #PB_Program_Wait)
          ...
          If erofs$ = ""
            ...
            Procedure ErofsPack(Parameter$)
              ...
              If ID
                ...
                If options$ <> ""
                  Err = RunConsole(bin$ + "erofs\mkfs.erofs.exe", options$, WorkingDirectory$, #PB_Program_Wait)
